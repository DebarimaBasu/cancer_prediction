...
export default function Chatbot() {
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await openai.chat.completions.create({
        model: "gpt-3.5-turbo",
        messages: [{ role: "system", content: "You are a medical assistant that helps users understand breast cancer predictions." },
                   ...newMessages.map(m => ({ role: m.sender === "user" ? "user" : "assistant", content: m.text }))],
        max_tokens: 100,
      });

      setMessages([...newMessages, { text: response.choices[0].message.content, sender: "bot" }]);
    } catch (error) {
  ...
  return (
    <div className="p-4 bg-gray-900 text-white rounded-lg w-96">
      ...
      <div className="h-60 overflow-y-auto border border-gray-700 p-2 rounded mb-2">
        {messages.map((msg, index) => (
          <p key={index} className={`p-2 ${msg.sender === "user" ? "text-right text-blue-400" : "text-left text-green-400"}`}>
            {msg.text}
          </p>
        ))}
      </div>
      ...
      />
      <button onClick={sendMessage} className="w-full mt-2 bg-blue-600 p-2 rounded hover:bg-blue-800">
        Send
      </button>
    </div>
  );
